...
//!
//! This module provides tools to test the cancelation safety of asynchronous methods.
//! Cancelation safety ensures that an asynchronous operation can be safely interrupted
//! and restarted without causing inconsistent or unexpected behavior.
//!
//! The library includes utilities to simulate cancelation scenarios and verify that
//! the tested async methods behave correctly under such conditions. This is particularly
//! useful when working with `tokio::select!` or other mechanisms that may cancel futures
//! during execution.
//!
//! ## Key Components
//!
//! - `cancelations`: A utility to test cancelation safety by simulating cancelation
//!   and restart of futures.
//! - `InterspersePending`: A wrapper for `AsyncRead` that interleaves `Poll::Pending`
//!   with actual reads, simulating scenarios where the future is not immediately ready.
//!
//! ## Examples
//!
//! ### Testing Cancelation Safety with `cancelations`
//!
